...
# Generic Browser -> StyledText + TalonFormatter patcher for all plugins
python3 - "$DIR" << 'PYEOF'
import os, subprocess, sys
root = sys.argv[1]
cp = os.path.join(root, 'asm.jar') + ':' + root
result = subprocess.run(
    ['java', '-cp', cp, 'GenericBrowserPatcher'],
